    ...
    : wxFrame(parent, id, title, pos, size, style), newWs(0), valueGiven(false)
{
    ...
    // Add button
    button = new wxButton(panel, BUTTON_OK, "Submit", wxPoint(50, 200), wxDefaultSize);
...
bool NumericInputFrame::isValueGiven(){
    return valueGiven;
}

void NumericInputFrame::setValueGiven(bool newValue){
    valueGiven = newValue;
}

long NumericInputFrame::getNewWs(){
    return newWs;
}

void NumericInputFrame::setNewWs(long newValue){
    newWs = newValue;
}
